...
import { AuthMode, IOptiGraphClient } from "@remkoj/optimizely-graph-client";
...
export async function getContentByPath(
  ...
  (client as IOptiGraphClient).updateAuthentication(isEnabled ? AuthMode.HMAC : AuthMode.Public);

  const draftQuery = isEnabled
    ? gql`
      {
        _or: [
          { _metadata: { changeset: { eq: "default" } } }
          { _metadata: { changeset: { eq: null }, status: { eq: "published" } } }
        ]
      }`
    : gql`{ _metadata: { status: { eq: "published" } } }`;
